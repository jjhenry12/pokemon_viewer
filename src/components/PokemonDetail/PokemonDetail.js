...
function PokemonDetail() {
  const { state : { descriptions, pokemons, selectedIndex } } = usePokemonProvider()
  const pokemon = pokemons[selectedIndex]
  const description = descriptions[selectedIndex]

  return (
    Boolean(pokemon) &&
      <section className="pokemon-detail" role="dialog" aria-labelledby="modal_title" aria-modal="true">
        <div className="pokemon-info">
          <img width="280" height="280" src={pokemon.sprites?.other?.dream_world?.front_default || ""} alt="" />
          <p>
            <h3 tabIndex="0" id="modal_title">{pokemon.name}</h3>
            {description.flavor_text_entries?.[1].flavor_text} {description.flavor_text_entries?.[2].flavor_text} {description.flavor_text_entries?.[3].flavor_text}
          </p>
        </div>
